...
public interface QuizAttemptRepo extends JpaRepository<QuizAttemptEntity, UUID> {
    ...
    Optional<QuizAttemptEntity> findByAssignmentIdAndStudentUsernameAndStatus(UUID assignmentId, String studentUsername,
            AttemptStatus status);

    List<QuizAttemptEntity> findByStatusAndExpiresAtLessThanEqual(AttemptStatus status, LocalDateTime time);

    List<QuizAttemptEntity> findByAssignmentIdAndStudentUsernameAndStatusNotOrderByAttemptNumberAsc(UUID assignmentId,
            String studentUsername, AttemptStatus status);
    ...
    int updateHeartbeat(@Param("id") UUID id, @Param("sessionToken") String sessionToken,
    ...
    int updateHeartbeatStealLock(@Param("id") UUID id, @Param("sessionToken") String sessionToken,
            @Param("now") LocalDateTime now);
    ...
    @Query(value = """
            ...
            LIMIT :limit
            """, nativeQuery = true)
    List<QuizAttemptEntity> findByQuizIdAndStatusAndCursor(UUID quizId, String status, UUID lastId,
