...
public interface QuizAssignmentRepo extends JpaRepository<QuizAssignmentEntity, UUID> {
        ...
        @Query(value = """
                        ...
                        """, nativeQuery = true)
        List<QuizAssignmentEntity> findByCourseIdAndDeletedAtIsNullAndStartTimeAndStatuses(UUID courseId,
                        LocalDateTime startTime, List<String> statuses);

        boolean existsByQuizIdAndDeletedAtIsNull(UUID quizId);

        List<QuizAssignmentEntity> findByStatusAndStartTimeLessThanEqualAndDeletedAtIsNull(AssignmentStatus status,
                        LocalDateTime time);

        List<QuizAssignmentEntity> findByStatusAndEndTimeNotNullAndEndTimeLessThanEqualAndDeletedAtIsNull(
