    ...
    @Query(value = """
            ...
            FROM (
                ...
                WHERE p.username = :username
                  AND (:hasCursor = FALSE OR (p.created_at < :cursorCreatedAt OR (p.created_at = :cursorCreatedAt AND p.id < :cursorId)))
                ...
                LEFT JOIN notification_group_read_status rs 
                       ON rs.notification_group_id = g.id AND rs.username = :username
                WHERE g.deleted_at IS NULL
                  AND (:hasRoles = TRUE AND EXISTS (
                      SELECT 1 FROM notification_group_target tgt 
                      WHERE tgt.notification_group_id = g.id AND tgt.role IN (:roles)
                  ))
                  AND (:hasCursor = FALSE OR (g.created_at < :cursorCreatedAt OR (g.created_at = :cursorCreatedAt AND g.id < :cursorId)))
            ) n
